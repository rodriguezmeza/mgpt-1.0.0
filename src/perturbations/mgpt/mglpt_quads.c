...
global_QRs QsR1R2_functions_ver3_LCDM(real eta, real ki);
...
//
//            R1aB += rsqr(rr)*psInterpolation(kk, PSLT, nPSLT)     // DESACTIVAR QUIZA PSLT
            R1aB += rsqr(rr)*psInterpolation_nr(kk, kPS, pPS, nPSLT) //CHECAR DESEMPEÑO DE ESTA OPCION
                    *wGL(pGL)[j]*ftmpR1;
        }
//
...
//    R1p = R1_function(eta, ki);
    R1p = R1_function_ver4(eta, ki);        // CHECAR EL DESEMPEÑO DE ESTA VERSION
...
global global_QRs QsR1R2_functions_driver_LCDM(real eta, real ki)
{
    ...
    qrs = QsR1R2_functions_ver3_LCDM(gd.xstop, ki);
