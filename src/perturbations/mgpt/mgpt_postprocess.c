/*==============================================================================
 MODULE: mgpt_postprocess.c			[mgpt]
...
global void PostProcessing(void)
{
    ...
    switch (process_int){
        ...
        case CLPTCORRELATIONS: CLPT_correlation_processing(); break;
...
local void postprocess_string_to_int(string process_str,int *process_int)
{
    *process_int = -1;
    ...
    if (strcmp(process_str,"clptcorrelation") == 0)        *process_int=CLPTCORRELATIONS;
}

#undef CLPTCORRELATIONS
#undef QFUNCTIONS
#undef PRD
#undef BIASTERMS
...
// P13 = 10./21. R1T[[ii, 2]] + 6./7. R2T[[ii, 2]] - sigma2L kT[[ii]] kT[[ii]] PSLT[[ii, 2]]
...
//
    ...
    if (nQsRsTable < 1)
        error("\n\nInputQsRsTable: nSolsTable = %d is absurd\n\n", nQsRsTable);
...
local real preMZAF(real q, real r, real mu, real fx, real hy)
{
// q^2 (2 Pi)^(-1/2) BDet[fx,hy] Exp[-0.5 (hy (q - mu r)^2 + fx (q^2 - 2 mu q r + r^2))]
    real func;
...
{
    real func;
    func = MUF(q, r, mu) + MA10F(q, r, mu);  // (* b1*)
    return (func);
}
...
{
    real func;
    func = MUUF(q, r, mu) + MU20F(q, r, mu);    //(* b2*)
    return (func);
}
...
{
    real func;
    func = MxiRF(q, r, mu) + MUUF(q, r, mu) + MU11F(q, r, mu); //(* b1^2*)
    return (func);
}
...
{
    real func;
    func = MUUF(q, r, mu) + MU11F(q, r, mu); //(* b1^2*)
    return (func);
}
...
// Use instead kminT and kmaxT or a range in between, for instance (cmd.kmin, cmd.kmax)
    kmin = 0.0001;
    kmax = 20.0;
    Nk = 12000;
    if (Nk==1)
