...
local real KR2_function(real ki, real rr, real xv)
{
    ...
    KR2 = (rr*xv*(1.0-rr*xv)/abskmq)*(KA - KB*rsqr(xv));
    return KR2;
}
...
//    pmax = kPos(PSLCDMtab+nPSTable-1);
    ...
    result=rlog(10.0)*(rsqr(gd.k)/FOURPI2)
            *psInterpolation_nr(gd.k, kPS, pPS, nPSLT)
...
local real KRI_function(real ki, real rr, real xv)
{
    ...
    KRI = (rsqr(rr)*(1.0-rsqr(xv))/abskmq)*(KA - KB*rsqr(xv));
    return KRI;
}
...
local real KR1p2_function(real ki, real rr, real xv)
{
    ...
    KR1p2 = (rsqr(rr)*(1.0-rr*xv)/abskmq)*(KA - KB*rsqr(xv));
    return KR1p2;
}
...
//    pmax = kPos(PSLCDMtab+nPSTable-10);
    ...
    result = 2.0*rlog(10.0)*rsqr(ki)*result/FOURPI2;
    return result;
}
